...
// The plan itself lives in the shared pricing namespace so this page and the
// home page can never drift apart on price, delay or deliverables.
const features = computed(() =>
  Object.values((tm('pricing.plans.landing.features') as Record<string, unknown>) || {}).map((feature) =>
    rt(feature)
  )
)
    ...
    <div class="relative mx-auto max-w-[1216px]">
        ...
        <div class="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_minmax(280px,340px)]">
          ...
          <div class="order-1 flex flex-col justify-center bg-[var(--bg-primary)] p-7 md:order-2 md:p-9">
            ...
            >
              ...
              <strong class="font-bold text-[var(--text-primary)]">{{ t('pricing.plans.landing.delay') }}</strong>
            </div>
            ...
            >
              {{ t('landing_offer.pricing.secondary_cta') }}
            </a>
          </div>
        </div>
      </article>
      ...
      <div class="mx-auto mt-10 max-w-[900px]">
        <p class="mb-4 text-center font-inter text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--text-muted)]">
          {{ t('landing_offer.pricing.alternatives_title') }}
        </p>
          ...
          >
            <div class="flex items-baseline justify-between gap-4">
              <h3 class="font-manrope text-lg font-semibold text-[var(--text-primary)]">
                {{ t(`landing_offer.pricing.alternatives.${key}.title`) }}
              </h3>
              <span class="font-manrope text-lg font-medium text-[var(--accent-gold)]">
                {{ t(`landing_offer.pricing.alternatives.${key}.price`) }}
              </span>
            </div>
            <p class="mt-2 font-inter text-sm leading-[1.65] text-[var(--text-secondary)]">
              {{ t(`landing_offer.pricing.alternatives.${key}.description`) }}
            </p>
          </article>
        </div>
      </div>
