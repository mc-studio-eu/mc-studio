...
const facts = ['delay', 'price', 'revisions'] as const
...
<template>
  ...
  >
    ...
    <div class="relative z-10 flex h-full flex-col items-center px-4 pb-12 sm:px-6 sm:pb-16">
      <LandingOfferNavbar variant="header" />

      <div
        class="mt-10 flex w-full max-w-[820px] flex-1 flex-col items-center justify-center gap-6 text-center sm:mt-12 md:gap-7"
      >
        <div
          class="inline-flex items-center gap-2 rounded-full border border-[rgba(255,255,255,0.1)] bg-black/40 px-3 py-1 backdrop-blur-md"
        >
          <span class="h-1.5 w-1.5 rounded-full bg-[var(--color-gold)]" />
          <span class="font-inter text-[11px] uppercase tracking-[0.16em] text-[rgba(255,255,255,0.8)]">
            {{ t('landing_offer.hero.badge') }}
          </span>
        </div>

        <h1
          class="landing-hero-title w-full font-manrope font-medium leading-[1.15] opacity-0 text-[clamp(1.5rem,6vw,1.75rem)] sm:text-[clamp(2rem,4.5vw,2.25rem)] md:text-[clamp(2.25rem,4vw,2.625rem)] lg:text-[2.75rem] xl:text-[3.125rem]"
        ...
        >
          ...
          <span class="landing-hero-gradient">{{ t('landing_offer.hero.title') }}</span>
        </h1>

        <p
          class="landing-hero-subtitle max-w-[620px] font-inter font-normal leading-[1.7] tracking-wide text-[rgba(255,255,255,0.9)] opacity-0 text-[clamp(0.875rem,2.5vw,1rem)] md:text-base lg:text-[1.0625rem]"
          style="filter: blur(15px); transform: translateY(15px)"
        >
          {{ t('landing_offer.hero.subtitle') }}
        </p>
        ...
        <div class="mt-1 flex w-full flex-col items-center justify-center gap-3 sm:flex-row sm:gap-5">
              ...
              <span class="text-slide-container h-[18px]">
                <span ref="primaryWrapper1" class="text-slide-wrapper">
                  <span class="text-slide-text h-[18px] font-inter text-sm font-semibold leading-[18px]">
                    {{ t('landing_offer.hero.primary_main') }}
                  </span>
                  <span class="text-slide-text h-[18px] font-inter text-sm font-semibold leading-[18px]">
                    {{ t('landing_offer.hero.primary_main') }}
                  </span>
              ...
              <span class="text-slide-container h-[12px]">
                <span ref="primaryWrapper2" class="text-slide-wrapper">
                  <span class="text-slide-text h-[12px] font-inter text-[9px] font-normal leading-[12px]">
                    {{ t('landing_offer.hero.primary_sub') }}
                  </span>
                  <span class="text-slide-text h-[12px] font-inter text-[9px] font-normal leading-[12px]">
                    {{ t('landing_offer.hero.primary_sub') }}
                  </span>
          ...
          <a
            ref="secondaryBtn"
            href="#tarif"
            class="flex items-center justify-center gap-2.5 rounded-xl border border-[rgba(255,255,255,0.12)] bg-black/50 px-5 py-2.5 font-inter text-sm font-medium text-[#f0eadb] no-underline transition-all duration-300 hover:bg-black/70"
          >
            <span class="text-slide-container h-[20px]">
              <span ref="secondaryWrapper" class="text-slide-wrapper">
                <span class="text-slide-text h-[20px] leading-[20px]">
                  {{ t('landing_offer.hero.secondary') }}
                </span>
                <span class="text-slide-text h-[20px] leading-[20px]">
                  {{ t('landing_offer.hero.secondary') }}
                </span>
        ...
        <div class="mt-4 grid w-full max-w-[620px] grid-cols-3 gap-3 sm:gap-4">
          <div
            v-for="fact in facts"
            :key="fact"
            class="landing-hero-fact flex flex-col items-center gap-1 rounded-xl border border-[rgba(255,255,255,0.1)] bg-black/35 px-2 py-3.5 backdrop-blur-xl sm:px-4"
          >
            <span class="font-inter text-[10px] uppercase tracking-[0.14em] text-[rgba(255,255,255,0.6)] sm:text-[11px]">
              {{ t(`landing_offer.hero.facts.${fact}.label`) }}
            </span>
            <span class="font-manrope text-base font-semibold text-[#ffffff] sm:text-xl">
              {{ t(`landing_offer.hero.facts.${fact}.value`) }}
            </span>
          </div>
        </div>

        <p class="font-inter text-xs text-[rgba(255,255,255,0.55)] sm:text-[13px]">
          {{ t('landing_offer.hero.note') }}
        </p>
      </div>
    </div>
  </section>
