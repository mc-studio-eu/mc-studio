...
<template>
  ...
  <section id="avis" class="py-12 px-6 sm:py-20 bg-[var(--bg-primary)] transition-colors duration-300 ease-out" @mouseenter="pauseAutoScroll" @mouseleave="resumeAutoScroll">
    <div class="max-w-[1216px] mx-auto">
      ...
      <!-- Testimonial Card -->
      <div class="w-full max-w-[460px] mx-auto p-6 bg-white border border-[#1a1a1a]/8 rounded-2xl h-[300px] sm:h-[230px] flex flex-col transition-colors duration-300 shadow-[0_4px_20px_rgba(26,26,26,0.05)]" v-if="currentTestimonial">
        <div class="flex flex-col sm:gap-0 gap-4 justify-between h-full">
          <p class="font-inter text-xs sm:text-sm leading-[1.7] text-[#1a1a1a] transition-colors duration-300">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-center gap-6 mt-8">
        <div class="flex gap-3">
          <button
              class="flex items-center justify-center w-11 h-[42px] bg-white border border-[#1a1a1a]/10 rounded-lg text-[var(--text-primary)] cursor-pointer transition-all duration-200 hover:border-[var(--color-gold)] hover:text-[var(--color-gold)]"
